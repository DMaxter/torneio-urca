...
async def delete_tournament(tournament_id: str, current_user=Depends(get_current_user)):
    ...
    teams = await db.db[TEAMS_COLLECTION].find({"tournament": ObjectId(tournament_id)}).to_list(1000)
    if any(len(t.get("players", [])) > 0 for t in teams):
        raise Error.bad_request("Não é possível eliminar um torneio com jogadores associados")
    await db.db[TOURNAMENTS_COLLECTION].delete_one({"_id": tournament["_id"]})
    get_logger().info(f"[{current_user['username']}] Deleted tournament '{tournament_id}'")
